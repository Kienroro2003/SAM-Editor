...
describe("AI and dashboard integration", () => {
  let pool;
  let app;
  let userId;
  let projectId;
  ...
  beforeEach(async () => {
    ...
    await pool.query(
      ...
      VALUES ($1, $2, $3, $4)
    `,
      [userId, "firebase-uid-ai-test", "ai-test@example.com", "AI Test User"]
    );
    ...
    await pool.query(
      ...
      VALUES ($1, $2, $3)
    `,
      [projectId, userId, "AI Coverage Project"]
    );
  ...
  it("generates AI recommendation and aggregates dashboard data", async () => {
    ...
    const targetLineNumber =
      lineResponse.body.data.lines.find((line) => line.status === "uncovered")?.lineNumber ||
      lineResponse.body.data.lines[0].lineNumber;
    ...
    expect(aiResponse.body.data.status).toBe("ready");
    expect(aiResponse.body.data.reason).toBeTruthy();
    ...
    expect(dashboardResponse.body.data.ai.stats.totalCount).toBeGreaterThanOrEqual(1);

    const cachedResponse = await request(app).get(
      `/api/ai/recommendations?scanFileId=${scanFileId}&lineNumber=${targetLineNumber}`
    );

    expect(cachedResponse.status).toBe(200);
    expect(cachedResponse.body.data.status).toBe("ready");
  });
});
