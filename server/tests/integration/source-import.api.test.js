...
describe("Source import scan flow", () => {
  let pool;
  let app;
  let userId;
  let projectId;
  ...
  beforeEach(async () => {
    ...
    await pool.query(
      ...
      VALUES ($1, $2, $3, $4)
    `,
      [userId, "firebase-source-import", "source@example.com", "Source User"]
    );
    ...
    await pool.query(
      ...
      VALUES ($1, $2, $3)
    `,
      [projectId, userId, "Source Import Project"]
    );
    ...
    const mockStorageService = {
      async uploadFilePath() {
        return {
          ...
          objectKey: "projects/mock/source.zip",
          sizeBytes: 1234,
          downloadUrl: "https://example.com/mock-source.zip"
        };
      }
    };
  ...
  it("imports source archive and persists source artifact metadata", async () => {
    const response = await request(app)
      .post("/api/scans/source-import")
      .field("projectId", projectId)
      .field("createdByUserId", userId)
      .attach("sourceArchive", fixturePath("source-sample.zip"));

    expect(response.status).toBe(201);
    expect(response.body.success).toBe(true);
    expect(response.body.data.scanId).toBeTruthy();
    expect(response.body.data.sourceArtifact.storageProvider).toBe("firebase");

    const artifactResult = await pool.query(
      `
      SELECT artifact_type, storage_provider, bucket_name, object_key, status
      FROM scan_artifacts
      WHERE scan_id = $1
      LIMIT 1
    `,
      [response.body.data.scanId]
    );

    expect(artifactResult.rows[0].artifact_type).toBe("source_zip");
    expect(artifactResult.rows[0].storage_provider).toBe("firebase");
    expect(artifactResult.rows[0].status).toBe("available");
  });
});
