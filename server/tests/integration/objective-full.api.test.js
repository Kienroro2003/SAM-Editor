...
describe("Full objective flow", () => {
  ...
  beforeEach(async () => {
    ...
    await pool.query(
      ...
      VALUES ($1, $2, $3, $4)
    `,
      [userId, "firebase-full-flow", "full@example.com", "Full Flow User"]
    );
    ...
    await pool.query(
      ...
      VALUES ($1, $2, $3)
    `,
      [projectId, userId, "Objective Full Project"]
    );
  ...
  it("supports coverage + requirement + test quality + AI insights workflow", async () => {
    ...
    const requirementImport = await request(app)
      .post(`/api/scans/${scanId}/requirements/import`)
      .send({
        requirements: [
          {
            requirementKey: "REQ-AUTH-001",
            title: "Math module basic behavior",
            mappings: [
              {
    ...
    expect(requirementImport.status).toBe(200);
    expect(requirementImport.body.data.coverage.summary.requirementsFound).toBe(1);

    const testCaseImport = await request(app)
      ...
      .send({
        testCases: [
          ...
          {
            testName: "handles invalid input",
            filePath: "tests/math.test.js",
            status: "skipped",
            assertions: 0,
            durationMs: 8
    ...
    expect(testCaseImport.status).toBe(200);
    expect(testCaseImport.body.data.quality.summary.total).toBe(2);
    expect(testCaseImport.body.data.quality.summary.weakCount).toBeGreaterThanOrEqual(1);

    const insightResponse = await request(app).get(`/api/ai/scans/${scanId}/insights`);

    expect(insightResponse.status).toBe(200);
    expect(insightResponse.body.data.summary.recommendationCount).toBeGreaterThanOrEqual(1);

    const dashboardResponse = await request(app).get(`/api/dashboard/${scanId}`);

    expect(dashboardResponse.status).toBe(200);
    expect(dashboardResponse.body.data.overview.scanId).toBe(scanId);
    expect(
      dashboardResponse.body.data.requirementCoverage.summary.requirementsFound
    ).toBeGreaterThanOrEqual(1);
    expect(
      dashboardResponse.body.data.testCaseQuality.summary.total
