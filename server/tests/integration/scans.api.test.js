...
describe("Scan API integration", () => {
  ...
  beforeEach(async () => {
    ...
    await pool.query(
      ...
      VALUES ($1, $2, $3, $4)
    `,
      [userId, "firebase-uid-test", "test@example.com", "Test User"]
    );
    ...
    await pool.query(
      ...
      VALUES ($1, $2, $3)
    `,
      [projectId, userId, "Coverage Test Project"]
    );
  ...
  it("ingests lcov and exposes overview, risk modules, and line coverage", async () => {
    const ingestResponse = await request(app)
      .post("/api/scans/lcov")
      .field("projectId", projectId)
      .field("createdByUserId", userId)
      .field("riskThreshold", "55")
    ...
    const overviewResponse = await request(app).get(`/api/scans/${scanId}/overview`);
    expect(overviewResponse.status).toBe(200);
    expect(overviewResponse.body.data.metrics.statement.found).toBe(8);
    expect(overviewResponse.body.data.metrics.statement.hit).toBe(5);

    const riskResponse = await request(app).get(
      `/api/scans/${scanId}/risk-modules?threshold=70&limit=10`
    );
    expect(riskResponse.status).toBe(200);
    expect(riskResponse.body.data.modules.length).toBeGreaterThan(0);
    ...
    expect(lineResponse.body.data.file.scanFileId).toBe(scanFileId);
    expect(lineResponse.body.data.lines.length).toBeGreaterThan(0);
  });

  it("returns 400 for malformed lcov content", async () => {
    const response = await request(app)
      .post("/api/scans/lcov")
      .field("projectId", projectId)
      .field("createdByUserId", userId)
      .attach("lcovReport", fixturePath("invalid-format.lcov"));

    expect(response.status).toBe(400);
    expect(response.body.success).toBe(false);
  });

  it("returns 400 when lcov file is missing", async () => {
    const response = await request(app)
      .post("/api/scans/lcov")
      .field("projectId", projectId)
      .field("createdByUserId", userId);
